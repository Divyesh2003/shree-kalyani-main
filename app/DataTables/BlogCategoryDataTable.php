        ...
        ->addColumn('action', function(BlogCategory $blogCategory) {
            return '<span style="display:flex">
                <a href="' . route('admin.blog.category.show', $blogCategory->id) . '" class="flex items-center mr-3 text-theme-23 " style="padding-right:8px; color:#997a00;"><svg  class="h-4 w-4 " xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />  <circle cx="12" cy="12" r="3" /></svg> </a>
                 ...
                 <form action="' . route('admin.blog.category.destroy', $blogCategory->id) . '" method="POST" class="m-0 delete">
                 ' . csrf_field() . '
                 ' . method_field("DELETE") . '
            ...
            ->addColumn('parent_id',function (BlogCategory $category)
            {
                if ($category->parent_id == "" || is_null($category->parent_id)) {
                    return "";
                }
